...
spl_autoload_register(function($class) {
    if (0 === strpos($class, 'Guzzle\AllPlayers')) {
        $class = str_replace('Guzzle\AllPlayers', '', $class);
        if ('\\' != DIRECTORY_SEPARATOR) {
            $class = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'Guzzle/AllPlayers' . DIRECTORY_SEPARATOR . str_replace('\\', DIRECTORY_SEPARATOR, $class) . '.php';
        } else {
            $class = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'Guzzle\AllPlayers' . DIRECTORY_SEPARATOR . $class . '.php';
        }
